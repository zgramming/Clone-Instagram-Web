...
function ExploreRecentHistoryItem({ index }: { index: number }) {
  return (
...
function ExploreRecentHistory() {
  ...
  const openModalClearHistory = () =>
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          You wont be able to undo this. Your search history will be cleared from
          <b>InstaClone.</b>
        </Text>
      ),
  ...
  return (
    <Stack spacing="md" className="pb-32">
      ...
      <Stack spacing="xs">
        {dummy.map((_, i) => (
          <ExploreRecentHistoryItem index={i} />
        ))}
      </Stack>
    </Stack>
  );
